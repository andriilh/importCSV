...
public class CSVLoader {
    ...
    public void loadCSV(String csvFile, String tableName){
        ...
        String query = "INSERT INTO "+ tableName
                                + " (kode_barang, kode_supplier, nama_barang, satuan_barang, harga_barang) VALUES"
                                + "(?,?,?,?,?)";
        try {
            ...
            while ((nextLine = reader.readNext()) != null) {
                lnNum++;
                // binding data kode_barang
                sql_statement.setString(1, nextLine[0]);
                // binding data kode_supplier
                sql_statement.setString(2, nextLine[1]);
                // binding data nama_barang
                sql_statement.setString(3, nextLine[2]);
                // binding data satuan_barang
                sql_statement.setString(4, nextLine[3]);
                // binding data harga_barang 
                // harus di convert ke integer terlebih dahulu
                sql_statement.setInt(5,Integer.parseInt(nextLine[4]));
